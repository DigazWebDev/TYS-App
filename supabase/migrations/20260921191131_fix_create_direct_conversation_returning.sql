-- INSERT ... RETURNING exige que a linha nova passe na policy de SELECT.
-- Essa policy depende dos membros, e o trigger que os cria é AFTER INSERT,
-- portanto ainda não são visíveis durante o RETURNING.
-- A RPC continua SECURITY INVOKER. O INSERT e o SELECT seguinte são
-- instruções separadas: o trigger já correu quando o SELECT começa.
